...
if (matchDomain(usa_adv_local_domains)) {
      ...
      window.setTimeout(function () {
        ...
        if (article) {
          ...
          if (article_id_dom) {
            ...
            getExtFetch(url_src, '', {"app-identifier": "http://com.news.screens", "device-type": "phone", "x-access-token": x_access_token}, fix_dowjones_fetch, data_ext_fetch_id++, [article]);
          }
        }
      }, 2000);
    }
            ...
            if (json) {
              ...
              if (pars_index) {
                ...
                if (hl_index) {
                  ...
                  let doc = parser.parseFromString('<div class="content mb-4">' + DOMPurify.sanitize(json[hl_index], dompurify_options) + '</div>', 'text/html');
                ...
                for (let i = pars_index; i < pars_index + 50; i++) {
                  ...
                  if (par && typeof par === 'string') {
                    if (par.match(/^\d{4}-\d{2}-/))
                      break;
                    let doc = parser.parseFromString('<p class="content mb-4"' + (style ? 'style="' + DOMPurify.sanitize(style, dompurify_options) + '"' : '') + '>' + par + '</p>', 'text/html');
    ...
    if (json_script) {
      try {
        ...
        if (json && json.props.pageProps.data.htmlContent) {
          ...
          let article = cs_param.article_sel || document.querySelector('div[class^="MainStory_storycontent__"], div[class^="blueprint-story-detail_storbpcontent_"]');
          if (json_text && article) {
  ...
  let paywall = document.querySelector(cs_param.paywall_sel || 'section > article > footer > a[data-referral="Paywall"]');
  if (paywall) {
    removeDOMElement(paywall);
    let json_script = getArticleJsonScript();
    if (json_script) {
